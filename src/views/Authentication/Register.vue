            ...
            <v-row align="center" justify="center">
                ...
                <v-col cols="7">
                    <v-card outlined max-width="500px">
                        <v-card-title>
                            Bienvenue sur InvoiceKit
                        </v-card-title>

                        <v-card-text>
                            <v-alert v-if="error" border="left" color="red" text>
                                Impossible de créer le compte, verifiez que les champs sont correctement remplis.
                            </v-alert>

                            <v-alert v-if="success" border="left" color="green" text>
                                Votre compte à bien été crée! Vous allez être redirigé...
                            </v-alert>
...
export default {
    ...
    data: () => ({
        rules: {
            ...
            username: [
                value => !!value || 'Obligatoire.',
                value => (value || '').length >= 5 || 'Incorrect.'
            ],
            password: [
                value => !!value || 'Obligatoire.',
                value => (value || '').length >= 8 || 'Incorrect.'
            ]
        },
        name: '',
        username: '',
