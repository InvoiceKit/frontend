<template>
	<v-card class="mt-10">
		<v-card-title>
            ...
            <v-text-field v-model="search" label="Chercher une adresse" solo-inverted hide-details />
		</v-card-title>
		
        <template v-if="syncedPayload.customerID">
            <v-data-table v-on:item-selected="handleSelection" :search="search" :headers="headers" :items="customer.addresses" :single-select="true" show-select />
        </template>
        <template v-else>
...
export default class InformationCard extends Vue {
    ...
    handleSelection(newValue: any) {
        this.syncedPayload.addressID = newValue.value ? newValue.item.id : undefined
    }
}
</script>